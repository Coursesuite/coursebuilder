...
var bootbox = window.bootbox || (function(document, $) {
	...
	that.getHighIndex = function (selector) {
	    ...
	    for (; i < max; i += 1) {
	        ...
	        if (e > 1000) console.log(elements[i]);
    ...
    // adds a class, calls callback (if set) after shown instead of after hidden
    that.bigwindow = function(/*str, label, cb*/) {
        var str   = "",
            label = _translate('OK'),
            cb    = null;
    ...
    that.dialog = function(str, handlers, options, kind, shown_callback) {
        var buttons    = "",
            callbacks  = [];
        ...
        [].forEach.call(document.querySelectorAll("div[tabindex='-1']"), function (el) {
	        // var index = el.getAttribute("tabindex");
	        // console.log("found", el, index);
	        el.removeAttribute("tabindex");
        });
        
        var parts = ["<div class='bootbox modal bootbox-" + kind + "' tarbindex='-1' style='overflow:hidden;z-index:1060'>"];
        ...
        var inp = div.find(".modal-body input[type=text]");
        if (inp.length) {
	        inp.attr("tabindex","1");
	        inp.focus();
	    }
        ...
        div.on('shown', function(e) {
            ...
            // if a shown-callback is defined, execute it
            // used by 'bigwindow'
	        if (typeof shown_callback === 'function') {
	        	shown_callback(e);
	        }
