...
function processString(strFile, gridToUse, applyHeader) {
	...
	for (var c=0;c<columns.length;c++) {
		...
		left = (function() {
			return left.join("").replace("<p></p>","");
		})();
		right = (function() {
			return right.join("").replace("<p></p>","");
		})();
...
// console.log("external", instruction);
				...
				case "center":
					tag.push("<div class='centered'>" + instruction[0] + "</div>");
					break;
				...
				case "p": tag.push("<p>" + instruction[0] + "</p>"); break;
				case "h1": tag.push("<h1>" + instruction[0] + "</h1>"); break;
				case "h2": tag.push("<h2>" + instruction[0] + "</h2>"); break;
				case "h3": tag.push("<h3>" + instruction[0] + "</h3>"); break;
				case "h4": tag.push("<h4>" + instruction[0] + "</h4>"); break;
				case "h5": tag.push("<h5>" + instruction[0] + "</h5>"); break;
				case "h6": tag.push("<h6>" + instruction[0] + "</h6>"); break;

				case "tag": // any html tag
					var tagName = instruction[0].split(".")[0],
						cssTag = (instruction[0].indexOf(".")==-1) ? "" : " class='" + instruction[0].split(".").slice(-1).pop() + "'";
					tag.push("<" + tagName + cssTag + ">" + instruction[1] + "</" + tagName + ">");
					break;
...
// console.log("sCompletionStatus", sCompletionStatus, "userScore", userScore, "__settings.course.passingScore", __settings.course.passingScore, "bIsCourseCompleted", bIsCourseCompleted, "sTemplate", sTemplate);
				...
				case "matchset":
					var myUniqueId = _nPageCurrent + "_" + _global_increment,
						quest = [], ans = {},
						k = 1; // first answer is index 1, because index 0 is "not yet answered"
					for (var i=0,j=instruction.length; i<j; i+=2) {
	...
	returnAr = [inp,container]; // inp.replace("\\ ","<br/>")
...
var size = parseInt(sze,10);
	...
	if (url.indexOf("vimeo.com") != -1) {
		url = "https://player.vimeo.com/video/" + url.substr(url.lastIndexOf("/")+1); // e.g. http://vimeo.com/13533846
