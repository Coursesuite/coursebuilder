...
Handlebars.registerHelper('drag2list', function(requiredAnswer, userResponse, context, readonly) {
	...
	//console.log('drag2list', requiredAnswer, userResponse, context, readonly);
...
function calcTest(bScored) {
    ...
    if (parent.updatePageStatus) {
        parent.updatePageStatus(_sCurrentPageId, _oTest.sPassStatus, _oTest.nTotalScore, bScored);
    }
}
...
function showResults() {
    // show the sumary information
	showQuestionList();
	calcTest(true); // drawing questions will have updated question object states, so recalc test
