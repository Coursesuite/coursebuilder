...
Handlebars.registerHelper('drag2list', function(requiredAnswer, userResponse, context) {
	...
	var ar = [];
		...
		outerDiv = $("<div />").addClass("drag2list").appendTo(_container);
		...
		for (var j=0; j<oQuestions.length; j++) {
			...
			switch (oQuestion.sType) { 
				...
				case "QuestionDragToList":
					...
					aList = jQuery.unique(aList);

					// ! drag to list ordering
...
function timeCheck() {
    ...
    if (_oTest.timeLimitExpired()) {
        ...
        $("#timeLimit").html(XsMsgTime + "<b>" + nTime + "</b>").show();
    }
}
...
console.log("rand", _oQuestion.aChoiceRand);
console.log("response", _oQuestion.sResponse);
console.log("answer", _oQuestion.sAnswer);
			if (_oQuestion.sResponse != "") {
				_answered = true;
				if (_oTest.bGraded && canRevealAnswer()) {
					_marked = true;
...
console.log("answers",i,"->" + aAnswers[i]);
			                for (j = 0; j < _oQuestion.aChoiceRand.length; j++) {
console.log("rand",j,_oQuestion.aChoiceRand[j],_oQuestion.aChoiceRand[j]-1);
			                    if (_oQuestion.aChoiceRand[j]-1 == aAnswers[i]) {
console.log("alpha",_aAlpha[aAnswers[i]], _aAlpha[j]);
			                        sCorrectIs += _aAlpha[aAnswers[i]];
			                        break;
			                    }
			                }
			            }
...
for (j=0;j<_oQuestion.aChoiceRand.length;j++) {
	for (var i=0;i<aAnswers.length;i++) {
		if (_oQuestion.aChoiceRand[j]==aAnswers[i]) {
			console.log("match",_oQuestion.aChoiceRand[j],aAnswers[i],String.fromCharCode(65 + j));
...
function showIntroductionPage(){
	...
	$("#question").html(Handlebars.getCompiledTemplate("quiz/intro", {
		"test" : {
			"title": _oTest.bQuiz ? XsQuizIntroTitle : XsTestIntroTitle,
			"instruction": _oTest.bQuiz ? XsIntroQuiz : XsIntroTest,
			"attempts": _attempts
		},
		"buttons": {
			"showgrade": {
...
function showResults() {
	...
	var _title = XsSummaryTitle.replace(/%%type/g, _oTest.sTestType),
		...
		_passed = false,
		sData = "",
		sObjectiveID = parent.getCurrentPageId();
...
function getTestStatusText() {
    ...
    return "<span class='quiz-status-wrap'><span class='quiz-status-mustdo'>" + line1MustDo + (line1MustDo.length ? "." : "") + "</span> <span class='quiz-status-hasdone'>" + line2HasDone + (line2HasDone.length ? "." : "") + "</span></span>";
}
    ...
    for (i=0; i<_nQuestionPools; i++) {
	    for(var j=0; j<_oTest.aQuestionPool[i].nDeliver; j++) {
	        ...
	        _questions.push({
	        	...
	        	"order": j,
	        	"index": nQTotal++,
	        	"text": _oTest.aQuestionPool[i].aQuestions[nOrder].sPrompt,
	        	"current": (_nCurrentQuestionPool == i && _nCurrentQuestion == j),
	        	"status": _oTest.aQuestionPool[i].aQuestions[nOrder].enStatus
	        });
	    }
	}
	$("#questionList").html(Handlebars.getCompiledTemplate("quiz/index", {
    	"test": {
...
$(document).ready(function () {
	var fn = location.href.split("?filename=")[1];
	initQuiz(fn);
	var quizLayout = $("[data='rp-quiz-layout']").layout({
