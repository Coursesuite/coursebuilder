...
Dim errCode
	...
	Dim cmd : cmd = Quote(Join(Array(nodeExe,nodeModule,DoubleQuote(inpPath),DoubleQuote(outPath))," "))
	...
	If errCode > 0 Then
		Response.Write "<span style='color:red'>LessC Error (" & errCode & ")</span>: " & ComSpec & cmd
' LessC Error: C:\Windows\system32\cmd.exe /c "C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node.exe C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node_modules\less\bin\lessc C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.less C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.css"
'  LessC Error: C:\Windows\system32\cmd.exe /c "C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node.exe C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node_modules\less\bin\lessc ""C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.less"" ""C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.css"""
'  LessC Error (1): C:\Windows\system32\cmd.exe /c "C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node.exe C:\inetpub\tardproof\phpapp\engine\bin\less.js-windows-v2.4.0\bin\node_modules\less\bin\lessc ""C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.less"" ""C:\inetpub\tardproof\courses - Copy\sandpit\MPM_LMS_Patient_Health_Records\SCO1\Layout\less\app.css"""
	End If
...
Function MyUserName
	...
	MyUserName = Trim("" & Request.Cookies("aspname"))
	Dim phpId : phpId = Trim("" & Request.Cookies("PHPSESSID"))
	if phpId > "" and MyUserName = "" then
		MyUserName = db.getScalarWithParams("SELECT name FROM plebs WHERE current_session_id={0}", Array(phpId), "")
	end if
	If MyUserName = "" Then MyUserName = "Unknown"
	' MyUserName = "coursebuilderdemo"
End Function
...
Function MyContainers(byval root)
	...
	If InStr(ul,"*") = 0 Then
		result = Split(ul, ",")
	Else
		result = IO.ArrayOfSubFolders(root)
	End If
	MyContainers = result
End Function

Function MyContainerIds(byval root, byval hide_subs, byval show_archived)
	Dim mc : mc = MyContainers(root)
	Dim results : results = ""
	Dim id : id = 0
	Dim sLoop, bLoop, bShow
	Dim subContainers : subContainers = split(db.getScalar("select group_concat(container) from plebs where container <> '*'", ""), ",")
	For sLoop = 0 to ubound(mc)
		bShow = true
		if mc(sLoop) = "archive" then
			if not show_archived then
				bShow = false
			end if
		end if
		if hide_subs and ubound(mc) > 0 then
			for bLoop = 0 to ubound(subContainers)
				If mc(sLoop) = subContainers(bLoop) then
					bShow = false
					exit for
				end if
			next
		...
		if bShow then
		id = db.getScalarWithParams("select id from container where name={0}", mc(sLoop), 0)
			If id > 0 Then
				results = results & id & ","
			end if
		end if
...
Function GetCoursePath(ByVal CourseId)
	Dim gcp
	gcp = db.getScalarWithParams("select path from CourseFolder where id = {0}", array(CourseId), "")
	gcp = Replace(gcp, "/courses_copy/", "/courses/")
	GetCoursePath = gcp
End Function
...
Dim cfg
	...
	If isobject(config) then
		cfg = "'" & db.sqlsafe(JSON.stringify(config, null, 4)) & "'"
	ElseIf config > "" Then
		cfg = "'" & db.sqlsafe(JSON.stringify(config, null, 4)) & "'"
	End If
...
' a personal container is where new courses are put. you have to move them out.
Function Main_PersonalContainer_Get()
	Main_PersonalContainer_Get = 0
	Dim t : t = Trim("" & Request.Cookies("aspname")) ' wow, this is asking for trouble
	if t > "" then
		Main_PersonalContainer_Get = db.getScalarWithParams("select id from container where name={0}", array(t), 0)
	end if
End Function

Sub Main_PersonalContainer_Set()
	Dim t : t = Trim("" & Request.Cookies("aspname")) ' wow, this is asking for trouble
	if t > "" then
		if db.getScalarWithParams("select count(1) from container where name={0}", array(t), 0) = 0 then
			call db.insert2("container", array("name", t))
		end if
	End If
End Sub
