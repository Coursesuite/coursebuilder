function __init__pages(objThis) {
	...
	if ($li.attr("id")) {
		...
		$.each(_fields, function(index,value) {
			...
			if (value == "contributeScore") inp.after(" % must be correct");
			if (value == "contributePercentage") inp.after(" % of the overall course score");
		...
		var div = $("<div />").addClass("field-row").append("<label class='force-inline-block'>Tools:</label>");
		div.appendTo(_container);
		$("<input type='button' value='Import Ninja Package' />").on("click", function(e) {
			PopupCenter("/app/import/ninja/" + _courseid + "/" +  $li.attr("id") + "/","ninjatools",480,640);
		}).appendTo(div);
		$("<input type='button' value='Upload HTML' />").on("click", function(e) {
			PopupCenter("/app/import/upload/" + _courseid + "/" +  $li.attr("id") + "/","ninjatools",480,640);
		}).appendTo(div);
