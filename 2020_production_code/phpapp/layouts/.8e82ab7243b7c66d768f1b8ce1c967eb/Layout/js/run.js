		...
		$("a[data-action]").click(function (event) {
			...
			switch (btnAction) {
				...
				case "toggle":
					...
					// tim: make sure the menu height matches the height of the container
					if ($(".ui-layout-west").hasClass("menu-open")) {
						$("#TOCContainer #TOC > .toc-child-wrapper").height($("#TOCContainer").height());
					}
					break;
...
function reBindThingsThatMightBeDynamicallyLoaded($page) {
	...
	$(".activity-match").each(function(index,el) {
		var $el = $(el), // the div wrapper for this whole activity
			...
			_state = ["I",0,[],0,0],
			_attempt = 0,
			_attempts = 2; // hard coded for now, zero based
		...
		function get_resit_text() {
			var a = (_attempts-_attempt);
			return (a===0) ? "No more attempts" : (a===1) ? "1 attempt remains" : a + " attempts remaining";
		}

		function handleResit() {
			// _attempt = Math.min(_attempts,++_attempt); // increment before applying min
			$(".resit-button p", $el).text(get_resit_text());
			if (_attempt === _attempts) {
				$(".resit-button button", $el).removeAttr("onclick");
			}
			reCalc(true);
		}
		function calcResit() {

			var $table = $(".activities", $el),
				remaining = $(".source>div[data-answer]", $table),
				$tfoot = $("tfoot", $table);
				console.log($table,remaining,$tfoot);

			if ($tfoot.length === 0 && remaining.length === 0 && _state[0] !== "P") {
				...
				td.setAttribute("class","centered resit-button");
				...
				p.textContent = get_resit_text();
				td.appendChild(p);
			}

		}

		function reCalc(force_popup) {
				...
				logic: if (logic.length && _state[0] !== "I") {

					if (_attempt > 0 && !force_popup) break logic;
					_attempt++; // because we just calculated the first resit
	...
	$(".activity-match-rows, .activity-match-cols").each(function(index,el) {
		...
		function handleResit() {
			// _attempt = Math.min(_attempts,++_attempt); // increment before applying min
			$(".columns p", $options).text(get_resit_text());
			if (_attempt === _attempts) {
				$(".columns button", $options).removeAttr("onclick");
			}
			reCalc(true);
		}
		function calcResit() {
			var remaining = $(".columns div[data-answer]", $options),
				button = $(".columns button", $options);

			if (button.length === 0 && remaining.length === 0 && _state[0] !== "P") {
				$(".columns", $options).addClass("centered").css({
					"column-count": "auto",
					"padding-top": "20%"
				});
				...
				button.textContent = "Check answers";
				button.classList.add("standard-button");
				$(".columns", $options).append(button);
				button = document.createElement("p");
				button.textContent = get_resit_text();
				$(".columns", $options).append(button);
			}
		...
		$(".drop",$el).droppable({
			...
			drop: function event_ondrop(event,ui) {
