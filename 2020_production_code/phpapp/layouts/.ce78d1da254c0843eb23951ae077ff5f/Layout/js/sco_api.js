...
var apiHandle = getAPI(); /* SCORM API */
...
function termCommunications() {
	scormCommit(); // spec says it's optional, but some implementations are wrong
	scormTerminate();
}
...
function setBookmark(sLocation) {
	...
	scormCommit(); // running commits during page navigation should be ok
}
...
// for when you know what you are doing
function forceCompletionStatus(sCompletion) {
	if (_sAPI == "API_1484_11") {
		scormSetValue("cmi.completion_status", sCompletion+"");
	} else if (_sAPI == "API") {
		scormSetValue("cmi.core.lesson_status", sCompletion+"");
...
function setScore(sScore) {
	...
	if (_sAPI == "API_1484_11") {
		...
		var wholeScore = Math.round(sScore * 100); // Technically a CMIDecimal, we don't need the accuracy, some api's are buggy
		if (wholeScore > 100) wholeScore = 100;
		...
		if ((sScore - 0) < 0) {
			...
			scormSetValue("cmi.core.score.raw", wholeScore+"");
		}
	}
}
...
*/
function setInteraction(sNum,sId,sType,sResponse,sCorrect,sResult,sWeight,nLatency,sDescription,sIdObjective,nObjectiveId) {
	var nLen, aPairs;
	...
	if (_sAPI == "API") {
		...
		/* modify answer format based on question type */
		switch (sType) {
			case "true-false": /* modify the responses from "true to "t", "false" to "f" */
			...
			case "choice":
			case "sequencing": /* see if there are [,] separators */
			...
			case "matching": /* see if there are [,] separators for the response */
				if (sResponse.indexOf('[,]') != -1) {
			...
			case "numeric": /* see if find a colon in the correct answer */
				if (sCorrect.indexOf() != -1) {
	...
	if (isNaN(parseInt(nObjectiveId,10))) nObjectiveId = 0;
	if (sIdObjective != null) scormSetValue(sInt + "objectives." + nObjectiveId + ".id", sIdObjective);
}
...
/*
* Get the index of an objective from its ID using a highest-first approach
*     ids don't have to be unique but we want to find the most recent one - the one with the higest index
* -> sStart - lowest possible index to search to
* -> sId - the ID of the objective that you would like to find
*
*  <- The index of the objective if we find a match, else null
*/
function getObjectiveIndex(sStart, sId) {
	for (var i = getObjectiveCount(); i >= ~~sStart; i--) {
		var current = scormGetValue("cmi.objectives." + i + ".id");
		if (current == sId) return (i + "");
	}
	return null;
}
...
function findAPI(win, apiName) {
	...
	try {
		while ( (win[apiName] == null) && (win.parent != null) && (win.parent != win) )
		{
			...
			if (findAPITries > 7)
			{
				console.log("findAPI gave up", win, apiName);
		...
		console.log("findAPI forced to stop at domain boundary", win, apiName);
		return null; // cross a domain boundary
	}
	return win[apiName];
}
...
function getAPI() {
   ...
   // start by looking for the scorm 2004 API in the current window / parents
   var theAPI = findAPI(window, 'API_1484_11');	

   // if the scorm 2004 API is null (could not be found in the current window)
   // and the current window has an opener window
   if ( (theAPI == null) && (window.opener != null) && (typeof(window.opener) != "undefined") )
   {
      // try to find the API in the current window’s opener
      theAPI = findAPI(window.opener, 'API_1484_11');
   }
   // if the scorm 2004 API has not been found, try the same approach for the scorm 1.2 api
   if (theAPI == null)
   {
   		// try to find the scorm 1.2 api in the current window / its parents
   		theAPI = findAPI(window, 'API');
		if ( (theAPI == null) && (window.opener != null) && (typeof(window.opener) != "undefined") )
		{
		  // try to find the scorm 1.2 API in the current window’s opener
		  theAPI = findAPI(window.opener, 'API');
		}
		// Set the variable which identifies us finding the scorm 1.2 api
		if (theAPI != null) {
			_sAPI = "API";
		}
   } else {
		// Set the variable which identifies us finding the scorm 2004 api
	   _sAPI = "API_1484_11";
   }
...
function showDebug(sDebug) {
	...
	if (_bDebug) {
		...
		var time = new Date(),
			t = ("0" + time.getHours()).slice(-2)   + ":" + ("0" + time.getMinutes()).slice(-2) + ":" + ("0" + time.getSeconds()).slice(-2) + " => ";

		_wDebug.document.write(t + sDebug + "<br>");
