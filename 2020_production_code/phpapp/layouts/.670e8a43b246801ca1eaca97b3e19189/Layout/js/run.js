...
function reBindThingsThatMightBeDynamicallyLoaded($page) {
	...
	$(".activity-match").each(function(index,el) {
		...
		if (_stored && _stored.length) {
			$.each(_stored.split(","), function(index,value) {
		...
		$(".drop",$el).droppable({
			...
			drop: function (event, ui) {
				...
				drops.each(function (index, d) {
					...
					if ($v.length && $v.attr("data-answer") == _req) {
						$d.switchClass("incorrect", "correct");
					} else if ($v.length && $v.attr("data-answer") != _req) {
						$d.switchClass("correct", "incorrect");
					} else {
						$d.switchClass("incorrect correct");
					}
				});
				var _sData = $.map(drops, function(o) { var r = $("div[data-answer]",o); return r.length ? r.attr("data-answer") : "-1" });
				setState($el.attr("data-scormid"), _sData.join(_avideSep));
			}
		});
	});
	...
	$(".activity-match-set").each(function(index,el) {
		...
		if (_stored && _stored.length) {
			$.each(_stored.split(_avideSep), function(index,value) {
...
function triggerFrumbox(uri, data, pos, size, fnAfter, fx) {
	...
	.frumbox({
        ...
        size: {                 // the size to open at (does not resize/scale automatically)
            ...
            left: $(window).scrollLeft() + ($(window).width() / 10),
