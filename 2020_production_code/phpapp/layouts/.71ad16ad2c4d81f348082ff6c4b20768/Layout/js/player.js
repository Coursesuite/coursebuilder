...
function loadCourse() {
    ...
    // set tool tips
	// SetTooltips();
  	document.body.classList.add("id-" + __settings.course.id);
...
function imageTag(filename, classes, attributes) {
	...
	if (filename.indexOf("://") !== -1) {
		...
		if (imgObj.alt) {
			img.attr("alt", imgObj.alt);
		}
	}
	if (classes) img.addClass(classes);
	if (attributes) img.attr(attributes); // a map
	if (imgObj.caption) {
		var fig = $("<figure>").append(img);
		var capt = $("<figcaption>").text(imgObj.caption).appendTo(fig);
		img = fig;
	}
