...
var _fields = ["title", "fileName", "type", "contribute", "contributeScore", "contributePercentage", "nav", "template","className"];
...
$(function () {
	...
	var $cbOptimise = $("#cbOptimise").change(function(e) {
		var link = $("#hrefDownload"),
			href = link.attr("href").replace("&OptimiseIsChecked=1","");
		if ($cbOptimise.is(":checked")) {
			href += "&OptimiseIsChecked=1";
		}
		link.attr("href", href);
	});
	...
	$("#tabs").tabs({
		activate: function(event, ui) {
			...
			} else if ( ui.newTab.index() == enTabs.Overrides) { // customise template
...
function processPages(obj){
	...
	if (!t.attr("title")) {
		...
		htmlTree.push(" className='" + (t.attr("className") || '') + "'>");
	}
