...
$(function() {
	$("#paste_screenshot p span").css("visibility","hidden");
...
function createImage(source) {
   ...
   pastedImage.onload = function() {
	...
	$(pasteCatcher).find("p").html("<i class='icon-spinner icon-spin icon-large'></i>");
	
	var data = new FormData();
	data.append("extn",imgExtn);
	data.append("base64",imgBase64);
	data.append("path", _media_path);
	$.ajax({
	    ...
	    success: function(url) {
			if (url.length) {
				var loc = location.href.split("&newfile=")[0]; // assumes it's the last one
				setTimeout("location.href += '&newfile=" + escape(url.split('/').slice(-1).pop()) + "';",753);
				// $("#hdnUrl").val(url);
