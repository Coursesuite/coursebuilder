...
class Library
	...
	public function exec(functionName, params)
		Dim func : set func = getFunction(functionName)
	...
	''
	''	Build a querystring based on the existing request.querystring, with substituted or appended parameters
	''	- Removes duplicate parameters
	''	
	function getQueryString(ByVal arrayOfParams)

		' A dictionary is a unique key store, so we can't duplicate params
		Dim dict
		Set dict = server.CreateObject("Scripting.Dictionary")
		
		' Add all the new params we want
		dim iLoop
		If UBound(arrayOfParams) > 0 Then
			For iLoop = 0 to UBound(arrayOfParams) Step 2
				If Not dict.Exists(arrayOfParams(iLoop)) Then
					dict.Add arrayOfParams(iLoop), arrayOfParams(iLoop+1)
				End If
			Next
		End if
		
		' Add all the existing params
		Dim qsArray, qsParam
		qsArray = Split(Trim("" & Request.QueryString), "&")
		For iLoop = 0 To UBound(qsArray)
			qsParam = Split(qsArray(iLoop), "=")
			If Not dict.Exists(qsParam(0)) Then
				dict.Add qsParam(0), qsParam(1)
			End If		
		Next
		
		' Convert Dictionary to Array
		Dim keys, items
		keys = dict.keys : items = dict.items
		ReDim qsArray(dict.Count - 1)
		For iLoop = 0 to dict.Count - 1
			qsArray(iLoop) = keys(iLoop) & "=" & Server.UrlEncode(items(iLoop))
		Next
		
		' Return joined array
		getQueryString = Join(qsArray, "&")
