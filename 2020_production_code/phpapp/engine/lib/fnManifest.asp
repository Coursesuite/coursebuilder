...
Sub fnManifest_UpdateImageManifest(byval path)
	Call fnManifest_UpdateImageManifest2(path, null)
end sub
Sub fnManifest_UpdateImageManifest2(byval path, byval captions)
Dim files, i, oJSON, oROW, aImages, w, h, img, ok, j
	...
	if captions > "" then
		Set captions = JSON.parse(captions)
	else
		Set captions = JSON.parse("{}")
	end if
	...
	If Config.FileSys.FolderExists(mappedFolder & "\SCO1\en-us\Content\media") Then
		...
		for i = 0 to ubound(files)
			...
			if ok then
				...
				on error resume next
					oROW.set "alt", captions.get(files(i)).alt
					oROW.set "caption", captions.get(files(i)).caption
				on error goto 0
