...
Class FileIORoutines
	...
	Function FileHash(byval fileToLoad)
		Dim objStream, ret
		ret = ""
		If Not Config.FileSys.FileExists(fileToLoad) Then
			FileHash = fileToLoad & " misssing"
			Exit Function
		End If
		Set objStream = server.CreateObject("adodb.stream")
	    objStream.Type = adTypeText
		...
		objStream.Open
		objStream.LoadFromFile fileToLoad
		If objStream.Size > 1 Then
			ret = objStream.ReadText
		End If
		objStream.Close
		Set objStream = Nothing
		FileHash = md5er.hash(ret)
	End Function		
	...
	Function ArrayOfTemplates()
		
		Dim n, v

		Dim dict : Set dict = server.CreateObject("Scripting.Dictionary")
		Dim rs : Set rs = db.getRS("SELECT `name`, md5(id) hash FROM Courses", Array())
		do while not rs.eof
			n = rs("hash")
			v = rs("name")
		...
		For Each sf in Config.FileSys.GetFolder(Config.LayoutsPath).SubFolders
			...
			Push2Array ar, "{""name"":""" & n & """,""folder"":""" & v & """},"
		Next
		Push2Array ar, "]"
		
		Dim str : str = Join(ar, "")
		str = Replace(str, ",]", "]")
		Set ArrayOfTemplates = JSON.parse(str)
	End Function
	...
	function GetOverriddenCourses(byval sExclude)
		...
		If uBound(arSql) > 0 Then
			...
			if right(filter_sql,5) <> " and " then
				...
				filter_sql = replace(filter_sql, " where and ", " where ")
			end if
	...
	sub CreateOverrideFolder(byval copy_from)
		CreateOverrideFolder2 copy_from, false
	end sub
	
	sub CreateOverrideFolder2(byval copy_from, byval bForce)
		Dim folderName : folderName = Config.LayoutsPath & "\." & md5er.hash(COURSE_ID)

		If bForce Or Not Config.FileSys.FolderExists(folderName) then
	...
	function GetOverriddenFilesForPath(ByVal relative_path)
		Dim basePath
		basePath = Config.BaselineThemePath
		Set GetOverriddenFilesForPath = GetOverriddenFilesForPath2(relative_path, basePath)
	end function
	function GetOverriddenFilesForPath2(ByVal relative_path, ByVal basePath)
		Dim rootPath : rootPath = Config.LayoutsPath & "\." & md5er.hash(COURSE_ID)

		Dim oSettings, templateRoot
		Call fnSettings_getCourseJsonObj(COURSE_ID, "config", oSettings)
		templateRoot = Config.LayoutsPath & "\" & oSettings.layout.template & replace(relative_path, "|", "\")
		...
		For Each fil in fold.files
			...
			oAttr.set "path", replace(relative_path & "\" & fil.name, "\", "|")
			if config.filesys.fileexists(templateRoot & "\" & fil.name) then ' file exists in template folder this override was copied from
				oAttr.set "overridden", true
			end if
			'hash1 = FileHash(rootpath & replace(relative_path,"|","\") & "\" & fil.name)
			'hash2 = FileHash(basePath & replace(relative_path,"|","\") & "\" & fil.name)
			'If hash1 <> hash2 then
			'	oAttr.set "changed", true
			'end if
			oObj.set "attr", oAttr
			Set oAttr = Nothing
			PushObj2Array oArray, oObj
		...
		Set GetOverriddenFilesForPath2 = oJson
		Set oJson = Nothing
	end function
