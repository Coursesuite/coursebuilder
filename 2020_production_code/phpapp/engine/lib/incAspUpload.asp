...
Class FreeASPUpload
	...
	public sub SaveOne(path, num, byref outFileName, byref outLocalFileName)
		SaveOneRoutine path, num, outFileName, outLocalFileName, false
	end sub
	
	public function GetOneFileName()
		if not UploadedYet Then Upload
		If UploadedFiles.Count > 0 Then
			Dim items : items = UploadedFiles.Items
			Dim fil : Set fil = items(0)
			GetOneFileName = fil.FileName
			Set fil = Nothing
		End If
	end function

	public function GetOneFileType()
		if not UploadedYet Then Upload
		If UploadedFiles.Count > 0 Then
			Dim items : items = UploadedFiles.Items
			Dim fil : Set fil = items(0)
			GetOneFileType = fil.ContentType
			Set fil = Nothing
		End If
	end function	
	''
	''	INPUTS
	''		path				the mapped folder to store into
	''		num					zero
	''	OUTPUTS
	''		outFileName			file name of item that was uploaded
	''		outLocalFileName	file name that was saved to disk (if modified)
	''		checkFolder			if true, checks for folder name instead of existing file name if/when modifying LocalFileName
	
	public sub SaveOneRoutine(path, num, byref outFileName, byref outLocalFileName, byval checkFolder)
		'response.write "<LI>" & path
		'response.write "<LI>" & path
		...
		'response.write "<LI>" & outFileName
			outLocalFileName = GetFileNameRoutine(path, outFileName, checkFolder)
		'response.write "<LI>" & outLocalFileName
			...
			StreamRequest.Position = fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			' response.write "path=" & path & ", outLocalFileName=" & outLocalFileName & ", outFileName=" & outFileName
...
Function GetFileName(strSaveToPath, FileName)
	GetFileName = GetFileNameRoutine(strSaveToPath, FileName,false)
end function

Function GetFileNameRoutine(strSaveToPath, FileName, byval matchfolder)
...
'tim: modified it so that it can look for a folder named the same as the filename we want to create, and increment the number from that
    Dim Counter
    Dim Flag, match
    Dim strTempFileName
    Dim FileExt
    ...
    Counter = 0
    filename = CheckFileNameCharacters(filename)
    p = instrrev(FileName, ".")
    FileExt = mid(FileName, p+1)
    strTempFileName = left(FileName, p-1)
 	If matchfolder then
	    NewFullPath = checkSlashes(strSaveToPath & "\" & left(FileName,instrrev(filename,".")-1))
    else
	    NewFullPath = checkSlashes(strSaveToPath & "\" & FileName)
    end if
    Flag = False
    rValue = ""
    ...
    Do Until Flag = True
    	If matchfolder then
    		match = objFSO.FolderExists(NewFullPath)
    	else
    		match = objFSO.FileExists(NewFullPath)
    	end if
        If match = False Then
            Flag = True
            If matchfolder then
	            rValue = Mid(NewFullPath, InstrRev(NewFullPath, "\") + 1) & "." & FileExt
            else
	            rValue = Mid(NewFullPath, InstrRev(NewFullPath, "\") + 1)
	        end if
        Else
            Counter = Counter + 1
            If matchfolder then
	            NewFullPath = checkSlashes(strSaveToPath & "\" & strTempFileName & "_" & Counter)
            else
	            NewFullPath = checkSlashes(strSaveToPath & "\" & strTempFileName & "_" & Counter & "." & FileExt)
            end if
        End If
    Loop
    
    GetFileNameRoutine = rValue
End Function

function checkSlashes(byval str)
	checkSlashes = SubstNoReg(str,"\\","\")
end function

Function CheckFileNameCharacters(byval str)
    str = SubstNoReg(str, "/", "_")
    str = SubstNoReg(str, ":", "_")
    str = SubstNoReg(str, "*", "_")
    str = SubstNoReg(str, "$", "_")
    str = SubstNoReg(str, "?", "_")
    ...
    str = SubstNoReg(str, ">", "_")
    str = SubstNoReg(str, " ", "_")
    CheckFileNameCharacters = SubstNoReg(str, "|", "_")
End Function
