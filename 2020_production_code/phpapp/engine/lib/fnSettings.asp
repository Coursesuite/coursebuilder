...
Dim dbGloss, dbRefs, dbHelp, dbCaptions
Dim bHasCustomOverride
	...
	Set rsCourse = db.getRS("select `config`, `glossary`, `help`, `references`, `captions` from courses where id={0}", course_id)
	
	dbGloss = Trim("" & rsCourse("glossary")) 
	dbRefs = Trim("" & rsCourse("references"))
	dbHelp = Trim("" & rsCourse("help"))
	dbCaptions = Trim("" & rsCourse("captions"))
	...
	End If
	Call IO.WriteUTF8WithoutBOM (mappedFolder & "\SCO1\Configuration\captions.json", dbCaptions)
	response.write "<li>Wrote captions"
	...
	Call fnManifest_UpdateImageManifest2(mappedFolder, dbCaptions)
